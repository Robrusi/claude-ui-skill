...
2. Do not add extra repository context unless it is necessary. Claude Code can inspect the repository itself.
3. Run the bundled helper script from this installed skill, outside the Codex sandbox, so Claude Code can use the user's existing local auth:
   - `bun run "$CODEX_HOME/skills/claude-ui/src/run-claude-ui.ts" -- "<user request>"`
   - If `CODEX_HOME` is unset, use `~/.codex/skills/claude-ui/src/run-claude-ui.ts`.
4. Wait for Claude Code to finish.
5. Inspect the resulting changes with:
   - `git status --short`
   - `git diff --stat`
6. Summarize what changed.
7. If no files changed, say so clearly.
